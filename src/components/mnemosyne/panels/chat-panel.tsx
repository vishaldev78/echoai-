                    ...
                    <AvatarFallback
                      className={
                        m.role === 'user'
                          ...
                          : 'bg-gradient-to-br from-emerald-500/25 to-emerald-500/25 text-xs font-semibold text-emerald-600 dark:text-emerald-300'
                      }
                    >
                    ...
                    {m.sources && m.sources.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        ...
                        {m.sources.map((s, i) => (
                          <span
                            key={i}
                            className="inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2 py-0.5 text-[10px] text-emerald-600 dark:text-emerald-300"
                          >
...
function Welcome({
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col items-center py-8 text-center">
      <span className="grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-emerald-500/15 to-emerald-500/15 ring-1 ring-emerald-500/25">
      ...
      <div className="mt-6 grid w-full gap-2 sm:grid-cols-2">
        {suggestions.map((q) => (
          <button
            key={q}
            onClick={() => onPick(q)}
            className="group flex items-center gap-2.5 rounded-xl border border-border/60 bg-background/50 px-3.5 py-3 text-left text-sm transition-all hover:border-emerald-500/40 hover:bg-emerald-500/[0.03]"
          >
            <Sparkles className="h-4 w-4 shrink-0 text-emerald-500/70 group-hover:text-emerald-500" />
