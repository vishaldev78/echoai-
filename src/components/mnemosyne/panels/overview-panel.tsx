    ...
    <div className="grid gap-5 lg:grid-cols-3">
      ...
      <div className="space-y-5 lg:col-span-2">
          ...
          <Card className="overflow-hidden border-amber-500/20 bg-gradient-to-br from-amber-500/[0.05] to-transparent">
            <CardContent className="p-6">
              ...
              <div className="mt-4 grid gap-2.5 sm:grid-cols-3">
                <FingerprintRow label="Writing voice" value={ts.writingStyle} />
                <FingerprintRow label="Problem solving" value={ts.problemSolving} />
                <FingerprintRow label="Preferences" value={ts.preferences} />
              </div>
            </CardContent>
          </Card>
        )}
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-sm font-semibold">
              <Sparkles className="h-4 w-4 text-amber-500" /> Featured memories
            </h3>
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-sm font-semibold">
              <Network className="h-4 w-4 text-amber-500" /> Memory graph
            </h3>
      ...
      <div className="space-y-5">
        <Card className="overflow-hidden border-amber-500/30 bg-gradient-to-br from-amber-500/10 to-emerald-500/10">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="mt-4 font-semibold">Ask the memory</h3>
            <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
              Converse in {profile.name.split(' ').slice(-1)[0]}&apos;s voice. Answers are grounded
              only in preserved memories.
            </p>
            <Button onClick={() => setTab('chat')} className="mt-4 w-full gap-2">
              Start conversation <ArrowRight className="h-3.5 w-3.5" />
            </Button>
            <div className="mt-3 flex flex-wrap justify-center gap-1.5">
              {['biggest failure', 'key discovery', 'what they rejected'].map((q) => (
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-sm font-semibold">
              <Clock className="h-4 w-4 text-amber-500" /> Recent timeline
            </h3>
