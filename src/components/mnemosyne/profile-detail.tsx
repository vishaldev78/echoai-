  ...
  if (loading || !profile) {
    return (
      <div className="flex h-[60vh] items-center justify-center text-muted-foreground">
        <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Awakening the memory…
      </div>
    )
  }
      ...
      <Card className="relative overflow-hidden border-border/60">
        ...
        <CardContent className="relative grid gap-6 p-6 sm:p-8 lg:grid-cols-[auto_1fr_auto] lg:items-center">
            ...
            {profile.thinkingStyle && (
              <div className="mt-4 flex items-start gap-2.5 rounded-lg border border-amber-500/20 bg-amber-500/[0.04] px-3 py-2.5">
                ...
                <p className="text-xs leading-relaxed text-muted-foreground">
                  <span className="font-medium text-foreground">Thinking fingerprint — </span>
          ...
          <div className="grid grid-cols-3 gap-3 lg:grid-cols-1 lg:gap-2">
            <HeaderStat icon={Layers} value={counts?.memories ?? 0} label="memories" />
            <HeaderStat icon={Network} value={counts?.documents ?? 0} label="documents" />
            <HeaderStat icon={Clock} value={counts?.timelineEvents ?? 0} label="events" />
          </div>
        </CardContent>
      </Card>
